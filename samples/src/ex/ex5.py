...
class xtuple(tuple):
    ...
    def __init__(self, s, compute = lambda n : n ):
        ...
        tuple.__init__(self)
        if isinstance(s, (IntType, LongType)):
    ...
    def __getitem__(self, s):
        if isinstance(s, (IntType, LongType)):
            if -len(self) <= s < 0:
                ...
                raise IndexError
        elif isinstance(s, SliceType):
            x = xrange(*s.indices(len(self)))   ## return xtuple
    ...
    def __add__(self, other):
        def tmp(n):
            if n < len(self): return self[n]
            else: return other[n-len(self)]
        return xtuple(len(self) + len(other), tmp)

    def __mul__(self, k):
        if not isinstance(k, (IntType, LongType)):
            raise TypeError
...
class LazyDictionary(object):
    ...
    def values(self):
        return xtuple(len(self), lambda n : self.load(self.keyList[n]))

    def items(self):
        return xtuple(len(self), lambda n : self.keyList[n],
                                            self.load(self.keyList[n]))
